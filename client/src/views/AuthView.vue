...
const authSchema = yup.object({
  ...
  phone: yup.string().when('type', {
    is: 'Register',
    then: (schema) => schema.required('Phone is required').matches(/^\+?[0-9]{10,14}$/, 'Phone number is invalid'),
    otherwise: (schema) => schema.notRequired(),
  }),
...
const onSubmit = handleSubmit(async ({type, password, phone, name, role, email}) => {
  const isRegister = type === 'Register'
  try {
    const {data} = await api.post<{ accessToken: string }>(isRegister ? '/register' : '/login', {
      email,
      password,
      phone,
      name,
...
<template>
  <main class="bg-surface-50 dark:bg-surface-950 p-10">
    <div class="bg-surface-0 dark:bg-surface-900 p-6 shadow rounded-border flex flex-col gap-5">
      <form @submit="onSubmit">
        ...
        <div v-show="type === 'Register'">
          ...
          <Select class="mt-5" v-model="role" :options="roleOptions" optionLabel="name"/>
        </div>
